	...
	while(M){
	    ...
	    for(int i=0;i<N;i++){
	        ...
	        if(op[0]=='+'){
	            ...
	            //vector<struct Lnode*> path;
	            Lroot = Ldelete(Lroot,low,high);
	            Lroot->max = Maxfix(Lroot);

	            /*for(int i=path.size()-1;i>=0;i--){
	                if(path[i]->left){
	                    path[i]->max=(path[i]->max>path[i]->left->max?path[i]->max:path[i]->left->max);
	                }
	                if(path[i]->right){
	                    path[i]->max=(path[i]->max>path[i]->right->max?path[i]->max:path[i]->right->max);
	                }
	            }*/
